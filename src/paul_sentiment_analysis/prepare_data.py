...
from .tokenize_data import score_to_sentiment, read_raw_data, tokenize_text
...
def prepare_tfidf_data(test_size=0.2, upsample=True):
    ...
    sklearn_ngram_tokenizer = create_sklearn_vectorizer(data['review'])
    if upsample:
        train = upsample_train(train)
    x_train = sklearn_vectorize(train['review'], sklearn_ngram_tokenizer)
    x_test = sklearn_vectorize(test['review'], sklearn_ngram_tokenizer)
